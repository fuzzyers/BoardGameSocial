...
const ResultTable = ({ game, members, event_id }: ResultTableProps) => {
    ...
    const availableMembers = members.filter((member) => !game.results.some((result) => result.user_id === member.id));
    ...
    const handleAddPlayer = async () => {
        ...
        if (!Number.isInteger(placementNumber) || placementNumber < 0) {
            return;
        }
    ...
    return (
        <>
            <View style={styles.container}>
                ...
                {game.results.length > 0 && (
                    <View style={styles.table}>
                        ...
                        {game.results.map((result, index) => (
                            <View key={result.user_id} style={[styles.row, index === game.results.length - 1 && styles.lastRow]}>
                                <View style={styles.playerColumn}>
                                    <Text style={styles.cellText}>{result.name}</Text>

                                    <Text style={styles.username}>@{result.username}</Text>
                                </View>

                                <View style={styles.placeColumn}>
                                    <Text style={[styles.cellText, result.placement === 0 && styles.dnfText]}>
                                        {result.placement === 0 ? "DNF" : result.placement}
                                    </Text>
                                </View>

                                <View style={styles.scoreColumn}>
                                    <Text style={styles.cellText}>{result.score}</Text>
                                </View>

                                <View style={styles.pointsColumn}>
                                    <Text style={styles.cellText}>{result.leaderboard_points}</Text>
                                </View>
                ...
                <View style={styles.addSection}>
                    ...
                    {availableMembers.length === 0 ? (
                        <Text style={styles.noPlayers}>All event members have been added.</Text>
                    ) : (
                        availableMembers.map((member) => (
                            <Pressable key={member.id} style={styles.memberButton} onPress={() => openAddPlayer(member)}>
                                <View>
            ...
            <Modal visible={selectedMember !== null} transparent animationType="fade" onRequestClose={closeModal}>
                    ...
                    <View style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            <View>
                                ...
                                <Text style={styles.modalPlayer}>{selectedMember?.name}</Text>
                            </View>
                        ...
                        <View style={styles.modalActions}>
                            ...
                            <Pressable
                                ...
                                disabled={saving}
                            >
                                <Text style={styles.saveButtonText}>{saving ? "Saving..." : "Save Result"}</Text>
                            </Pressable>
                        </View>
                    </View>
                </View>
